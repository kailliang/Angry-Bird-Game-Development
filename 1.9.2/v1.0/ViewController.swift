...
class ViewController: UIViewController, subviewDelegate{
    ...
    func changeSomething() {
        ...
        DispatchQueue.main.asyncAfter(deadline: when) {
           ...
           //self.newbird.image = UIImage(named: "bird2.png")
            //self.newbird.frame = CGRect(x: self.W - self.H/5 - 20, y: 0, width: self.H/5, height: self.H/5)
            //self.view.addSubview(self.newbird)
            self.birdsarray.append(bird )
            
           // for newbird in self.birdsarray{self.view.addSubview(newbird)}
                    }}}
